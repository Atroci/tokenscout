...
Extraia design tokens de um site renderizado e no ar. Não de um arquivo CSS
nem de uma exportação do Figma, e sim de uma página real, do jeito que o
...
Aponte o tokenscout para uma URL no ar e receba de volta um conjunto de design
tokens fiel e reutilizável (paleta, escala tipográfica, escala de espaçamento e
movimento) além dos recursos de imagem do site, prontos para iniciar um
redesign. Ele lê o que o navegador realmente pinta, então funciona em qualquer
stack, framework ou sem framework nenhum.

> Status: v0.1.0, open core. Já entregue: a camada de cor sem dependências
> (parse · sRGB→Lab · ΔE76 · clusterização por percepção), totalmente testada.
> A seguir: a camada de extração de sites no ar (estilos computados, coleta de
> recursos, captura de animação) e os redutores de tipo/espaçamento. Veja
...
- **No ar, não na origem.** O que chega à tela do usuário ≠ o que está na
  ...
  tempo de execução).
- **Clusterização por percepção.** `#3a7bd5`, `#3b7cd6` e `rgb(58,123,213)`
  são três strings, mas uma só cor. O tokenscout as clusteriza em CIELAB por
  ΔE76, transformando "47 cores declaradas → 9 cores reais".
...
um pacote extract que controla um navegador headless. Detalhe completo em
